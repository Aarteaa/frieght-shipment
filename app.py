...
with tabs[4]:
    ...
    snapshot_date = df["promised_delivery_date"].max()
    in_transit["past_promised"] = in_transit["promised_delivery_date"] < snapshot_date
    pct_at_risk = in_transit["past_promised"].mean() if len(in_transit) else 0

    st.metric("In-transit shipments past promised date (illustrative, static snapshot)",
              f"{pct_at_risk:.0%}",
              help="This is a static file, so this is illustrative only -- in production "
                   "this is recomputed daily against the real current date.")
